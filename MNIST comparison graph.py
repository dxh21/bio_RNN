...
class RNNSTP(nn.Module):
    
    def __init__(self, input_size, hidden_size, num_layers, num_classes, complexity):
        super(RNNSTP, self).__init__()
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.lstm = STP(input_size, hidden_size, complexity, 0.9, 0.1).to(device)
        ...
        for i, (images, labels) in enumerate(loaders['train']):
            ...
            for n in range(images.size(dim=0)):         # Use this loop to spiralise the image 
                for image in images[n,0:1,:,:]: 
                    ...
                    #image = baseindexing(3, input_size, 1, image)  
                    p[n,0,:,:] = spiralimage
...
if __name__ == '__main__':
    ...
    test_accuracies_gru = []
    test_accuracies_STP = []
    test_accuracies_STP_spiral = []

    loss_func = nn.CrossEntropyLoss()

    from torch import optim

    for a in [4,8,14,16,28,49]:
        sequence_length = int((28*28)/a)
        input_size = a
        model = RNNSTP(input_size, hidden_size, num_layers, num_classes, "poor").to(device)  
        optimizer = optim.Adam(model.parameters(), lr = 0.01)  
        print(model)
        train(num_epochs, model, loaders)
        test_accuracies_STP.append(evaluate(model))

    for a in [4,8,14,16,28,49]:
        sequence_length = int((28*28)/a)
        input_size = a
        model = RNNSTP(input_size, hidden_size, num_layers, num_classes, "rich").to(device)  
        optimizer = optim.Adam(model.parameters(), lr = 0.01)  
        print(model)
        trainspiral(num_epochs, model, loaders)
        test_accuracies_STP_spiral.append(evaluatespiral(model))
    ...
    '''for a in [4,8,14,16,28,49]:
        ...
        print(model)
        train(num_epochs, model, loaders)
        test_accuracies_rnn.append(evaluate(model))

    for a in [4,8,14,16,28,49]:
        sequence_length = int((28*28)/a)
        input_size = a
        model = RNNGRU(input_size, hidden_size, num_layers, num_classes).to(device)  
        optimizer = optim.Adam(model.parameters(), lr = 0.01)  
        print(model)
        train(num_epochs, model, loaders)
        test_accuracies_gru.append(evaluate(model))'''
    ...
    test_accuracies_STP_array = np.array(test_accuracies_STP)
    test_accuracies_STP_spiral_array = np.array(test_accuracies_STP_spiral)
    #np.save("test_accuracies_STP_array", test_accuracies_STP_array) 
    #np.save("test_accuracies_STP_spiral_array", test_accuracies_STP_spiral_array)

    plt.plot([4,8,14,16,28,49], test_accuracies_rnn)
    plt.plot([4,8,14,16,28,49], test_accuracies_gru) 
    plt.plot([4,8,14,16,28,49], test_accuracies_STP)
    plt.plot([4,8,14,16,28,49], test_accuracies_STP_spiral)
    plt.xlabel("Input size ([4,8,14,16,28,49])")
    plt.ylabel("Test Accuracies")
    plt.legend(["STP", "STP spiral"])
    plt.show()
